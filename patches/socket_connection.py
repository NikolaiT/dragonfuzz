...
class SocketConnection(itarget_connection.ITargetConnection):
    ...
    """
    _PROTOCOLS = ["tcp", "ssl", "udp", "raw-l2", "raw-l3", "wifi"]
    ...
    def open(self):
        ...
        if self.proto == "tcp" or self.proto == "ssl":
            ...
            if not self.wifi_dev:
                raise ValueError('Please add a wifi_dev property to your connection when using protocol "wifi"')
            ETH_P_ALL = 3
            self._sock = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(ETH_P_ALL))
            self._sock.bind((self.wifi_dev, ETH_P_ALL))
        else:
            raise exception.SullyRuntimeError("INVALID PROTOCOL SPECIFIED: %s" % self.proto)
